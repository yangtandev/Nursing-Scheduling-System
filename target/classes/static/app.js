...
function refreshTimeTable() {
    $.getJSON("/timeTable", function (timeTable) {
        ...
        headerRowByShift.append($("<th>Dates</th>"));
        ...
        headerRowByName.append($("<th>Dates</th>"));
        ...
        headerRowByStaffGroup.append($("<th>Dates</th>"));
        ...
        $.each(timeTable.datesList, (index, dates) => {
            const rowByShift = $("<tr>").appendTo(tbodyByShift);
            rowByShift
                    .append($(`<th class="align-middle"/>`)
                            .append($("<span/>").text(`
                    ${dates.date.charAt(0) + dates.date.slice(1).toLowerCase()}
                    ${moment(dates.startTime, "HH:mm:ss").format("HH:mm")}
                    -
                    ${moment(dates.endTime, "HH:mm:ss").format("HH:mm")}
                `)
                                    .append($(`<button type="button" class="ml-2 mb-1 btn btn-light btn-sm p-1"/>`)
                                            .append($(`<small class="fas fa-trash"/>`)
                                            ).click(() => deleteDates(dates)))));
            ...
            rowByName
                ...
                `)));
            $.each(timeTable.shiftList, (index, shift) => {
                rowByShift.append($("<td/>").prop("id", `dates${dates.id}shift${shift.id}`));
            });
            ...
            $.each(nameList, (index, name) => {
                rowByName.append($("<td/>").prop("id", `dates${dates.id}name${convertToId(name)}`));
            });

            $.each(staffGroupList, (index, staffGroup) => {
                rowByStaffGroup.append($("<td/>").prop("id", `dates${dates.id}staffGroup${convertToId(staffGroup)}`));
            });
        });
        ...
        $.each(timeTable.staffList, (index, staff) => {
            ...
            if (staff.dates == null || staff.shift == null) {
                unassignedStaffs.append(staffElement);
            } else {
                $(`#dates${staff.dates.id}shift${staff.shift.id}`).append(staffElement);
                $(`#dates${staff.dates.id}name${convertToId(staff.name)}`).append(staffElementWithoutDelete.clone());
                $(`#dates${staff.dates.id}staffGroup${convertToId(staff.staffGroup)}`).append(staffElementWithoutDelete.clone());
            }
        });
    });
}
...
function addDates() {
    $.post("/datess", JSON.stringify({
        "date": $("#dates_date").val().trim().toUpperCase(),
        "startTime": $("#dates_startTime").val().trim(),
        "endTime": $("#dates_endTime").val().trim()
    }), function () {
        refreshTimeTable();
    }).fail(function(xhr, ajaxOptions, thrownError) {
        showError("Adding dates failed.", xhr);
    });
    $('#datesDialog').modal('toggle');
}

function deleteDates(dates) {
    $.delete("/datess/" + dates.id, function () {
        refreshTimeTable();
    }).fail(function(xhr, ajaxOptions, thrownError) {
        showError("Deleting dates (" + dates.name + ") failed.", xhr);
    });
}
...
$(document).ready( function() {
    ...
    $("#addDatesSubmitButton").click(function() {
        addDates();
    });
